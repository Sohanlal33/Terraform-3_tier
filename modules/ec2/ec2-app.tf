resource "aws_launch_template" "app" {
  ...
  user_data           = file("${path.module}/../../userdata/app-tier-user-data.sh")
...
resource "aws_lb" "app_internal" {
  name               = "${var.project_name}-app-internal-alb"
  internal           = true
...
resource "aws_autoscaling_group" "app" {
  ...
  tag { key = "Name" value = "${var.project_name}-app-asg" propagate_at_launch = true }
}
